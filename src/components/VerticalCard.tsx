...
export const VerticalCard = ({ title, imageSrc, link }: CardTypes) => {
  return (
    <Link to={link || ''}>
      <div
        className={`group relative mx-3 flex h-[calc(${VERTICAL_CARD_SIZE.height}+${CARD_PARAGRAPH_HEIGHT})] w-[${VERTICAL_CARD_SIZE.width}] ${link ? 'cursor-pointer' : 'cursor-default'} flex-col`}
      >
        <img
          src={imageSrc}
        ...
        ></img>
        {/* <div
          className={`h-[${VERTICAL_CARD_SIZE.height}] w-[${VERTICAL_CARD_SIZE.width}] bg-neutral-50`}
        ></div> */}
        <p
